...
function initial_convergence(d, state, method::AbstractOptimizer, initial_x, options)
  ...
  return maximum(abs, gradient(d)) <= options.g_abstol, stopped
end
...
function optimize(
  ...
  return MultivariateOptimizationResults{typeof(method),Tx,typeof(x_abschange(state)),Tf,typeof(tr),Bool,typeof(stopped_by)}(
    method,
    initial_x,
