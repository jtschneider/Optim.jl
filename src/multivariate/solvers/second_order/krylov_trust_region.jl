...
function update_state!(objective::TwiceDifferentiableHV, state::KrylovTrustRegionState, method::KrylovTrustRegion)
...
function assess_convergence(state::KrylovTrustRegionState, d, options::Options)
  ...
  # Relative Tolerance
  if abs(state.f_diff) <
     max(options.f_reltol * (abs(value(d)) + options.f_reltol), eps(abs(value(d)) + abs(state.f_x_previous)))
    f_converged = true
  end
