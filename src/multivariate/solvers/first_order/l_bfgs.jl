...
function LBFGS(;
  ...
  return LBFGS(Int(m), _alphaguess(alphaguess), linesearch, P, precondprep, manifold, scaleinvH0)
end
...
function reset!(method, state::LBFGSState, obj, x)
  ...
  return state.pseudo_iteration = 0
end
function initial_state(method::LBFGS, options, d, initial_x)
  ...
  return LBFGSState(
    ...
    [similar(initial_x) for i in 1:method.m], # Store changes in position in state.dx_history
    [eltype(gradient(d))(NaN) .* gradient(d) for i in 1:method.m], # Store changes in position in state.dg_history
...
function trace!(tr, d, state, iteration, method::LBFGS, options, curr_time=time())
  return common_trace!(tr, d, state, iteration, method, options, curr_time)
end
