...
function NGMRES(;
  ...
  return NGMRES(_alphaguess(alphaguess), linesearch, manifold, nlprecon, nlpreconopts, ϵ0, wmax)
end
...
function OACCEL(;
  ...
  @assert manifold == nlprecon.manifold
  return OACCEL(_alphaguess(alphaguess), linesearch, manifold, nlprecon, nlpreconopts, ϵ0, wmax)
end
...
function nlprecon_post_accelerate!(d, state::NGMRESState{X,T}, method::LBFGS) where {X} where {T}
  state.nlpreconstate.pseudo_iteration += 1
  return update_h!(d, state.nlpreconstate, method)
end
...
function update_state!(d, state::NGMRESState{X,T}, method::AbstractNGMRES) where {X} where {T}
  ...
  if any(isnan, α)
    ...
    state.xA .= (1.0 - sum(α)) .* vec(state.x) .+ sum(state.X[:, k] * α[k] for k in 1:curw)
  ...
  if real(dot(state.s, gP)) ≥ 0 || !isfinite(real(dot(state.s, gP)))
    ...
    if typeof(method.alphaguess!) <: LineSearches.InitialConstantChange
      ...
      if isdefined(nlprec, :alphaguess!) && typeof(nlprec.alphaguess!) <: LineSearches.InitialConstantChange
  ...
  return lssuccess == false # Break on linesearch error
end
