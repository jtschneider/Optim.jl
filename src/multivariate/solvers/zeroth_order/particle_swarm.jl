...
function update_state!(f, state::ParticleSwarmState{T}, method::ParticleSwarm) where {T}
  ...
  f.F = housekeeping!(state.score, state.best_score, state.X, state.X_best, state.x, value(f), state.n_particles)
  # Elitist Learning:
  ...
  if state.limit_search_space
    state.x_learn[random_index] =
      state.x_learn[random_index] + (state.upper[random_index] - state.lower[random_index]) / 3.0 * r3
  else
...
function update_swarm!(X::AbstractArray{Tx}, X_best, best_point, n, n_particles, V, w, c1, c2) where {Tx}
...
function housekeeping!(score, best_score, X, X_best, best_point, F, n_particles)
  n = size(X, 1)
  for i in 1:n_particles
