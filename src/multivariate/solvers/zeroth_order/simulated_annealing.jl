...
"""
SimulatedAnnealing(; neighbor=default_neighbor!, temperature=log_temperature, keep_best::Bool=true) =
...
function initial_state(method::SimulatedAnnealing, options, d, initial_x::AbstractArray{T}) where {T}
  ...
  return SimulatedAnnealingState(copy(best_x), 1, best_x, copy(initial_x), value(d), value(d))
end
