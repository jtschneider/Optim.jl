...
promote_objtype(method::IPNewton, x, autodiff::Symbol, inplace::Bool, f) =
  TwiceDifferentiable(f, x, real(zero(eltype(x))); autodiff=autodiff)
promote_objtype(method::IPNewton, x, autodiff::Symbol, inplace::Bool, f, g) =
  TwiceDifferentiable(f, g, x, real(zero(eltype(x))); inplace=inplace, autodiff=autodiff)
promote_objtype(method::IPNewton, x, autodiff::Symbol, inplace::Bool, f, g, h) =
...
"""
IPNewton(; linesearch::Function=backtrack_constrained_grad, μ0::Union{Symbol,Number}=:auto, show_linesearch::Bool=false) =
...
function update_fg!(d, constraints::TwiceDifferentiableConstraints, state, method::IPNewton)
  ...
  return update_gtilde!(d, constraints, state, method)
end
...
function update_state!(
  ...
  state.alpha = method.linesearch!(ϕ, T(1), αmax, qp; show_linesearch=method.show_linesearch)
...
function default_options(method::ConstrainedOptimizer)
  return (; allow_f_increases=true, successive_f_tol=2)
end
...
function gf(bounds::ConstraintBounds, state)
  ...
  μsinv = μ * [bounds.σx ./ bstate.slack_x; bounds.σc ./ bstate.slack_c]
  gtildeμ = state.gtilde - jacobianI(state, bounds)' * μsinv
  return [gtildeμ; state.bgrad.λxE; state.bgrad.λcE]
end
