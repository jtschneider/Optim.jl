...
function BarrierStateVars{T}(bounds::ConstraintBounds) where {T}
  ...
  return sv = BarrierStateVars{T}(slack_x, slack_c, λx, λc, λxE, λcE)
end
...
Base.similar(bstate::BarrierStateVars) = BarrierStateVars(
  similar(bstate.slack_x),
...
Base.copy(bstate::BarrierStateVars) = BarrierStateVars(
  copy(bstate.slack_x),
  copy(bstate.slack_c),
  copy(bstate.λx),
  copy(bstate.λc),
...
Base.convert(::Type{BarrierStateVars{T}}, bstate::BarrierStateVars) where {T} = BarrierStateVars(
  convert(Array{T}, bstate.slack_x),
...
Base.isempty(bstate::BarrierStateVars) =
  isempty(bstate.slack_x) & isempty(bstate.slack_c) & isempty(bstate.λxE) & isempty(bstate.λcE)
...
Base.:(==)(v::BarrierStateVars, w::BarrierStateVars) =
  v.slack_x == w.slack_x && v.slack_c == w.slack_c && v.λx == w.λx && v.λc == w.λc && v.λxE == w.λxE && v.λcE == w.λcE
...
function dot(v::BarrierStateVars, w::BarrierStateVars)
  return dot(v.slack_x, w.slack_x) +
...
function norm(b::BarrierStateVars, p::Real)
  return norm(b.slack_x, p) + norm(b.slack_c, p) + norm(b.λx, p) + norm(b.λc, p) + norm(b.λxE, p) + norm(b.λcE, p)
end
...
Base.convert(::Type{BarrierLineSearch{T}}, bsl::BarrierLineSearch) where {T} =
  BarrierLineSearch(convert(Vector{T}, bsl.c), convert(BarrierStateVars{T}, bsl.bstate))
...
Base.convert(::Type{BarrierLineSearchGrad{T}}, bsl::BarrierLineSearchGrad) where {T} = BarrierLineSearchGrad(
  convert(Vector{T}, bsl.c),
...
function initial_convergence(d, state, method::ConstrainedOptimizer, initial_x, options)
  ...
  return norm(gradient(d), Inf) + norm(state.bgrad, Inf) < options.g_abstol, stopped
end
...
function optimize(
  ...
  return optimize(d, lower, upper, initial_x, method, options)
end
function optimize(
  f,
  g,
...
function optimize(
  ...
  return optimize(d, lower, upper, initial_x, IPNewton(), options)
end
...
function optimize(
  ...
  constraints = TwiceDifferentiableConstraints((c, x) -> nothing, (J, x) -> nothing, (H, x, λ) -> nothing, bounds)

  state = initial_state(method, options, twicediffed, constraints, initial_x)

  return optimize(twicediffed, constraints, initial_x, method, options, state)
end
function optimize(
  d::AbstractObjective,
  ...
  while !converged && !stopped && iteration < options.iterations
    ...
    x_converged, f_converged, g_converged, f_increased = assess_convergence(state, d, options)
...
function initialize_μ_λ!(state, bounds::ConstraintBounds, μ0::Union{Number,Symbol}, β::Number=1 // 100)
  return initialize_μ_λ!(state, bounds, nothing, μ0, β)
end
...
function sigma(bounds::ConstraintBounds)
  return [bounds.σx; bounds.σc]  # don't include σz
end
...
"""
hessianI(x, constraints, λcI, μ) = hessianI!(zeros(eltype(x), length(x), length(x)), x, constraints, λcI, μ)
...
function lagrangian_linefunc(αs, d, constraints, state)
  return _lagrangian_linefunc(αs, d, constraints, state)[2]
end
...
function _lagrangian_linefunc(αs, d, constraints, state)
  ...
  return lagrangian(d, constraints.bounds, state.x_ls, b_ls.c, b_ls.bstate, state.μ)
end
alphax(α::Number) = α
...
function lagrangian_lineslope!(αs, d, constraints, state, method::IPOptimizer{typeof(backtrack_constrained_grad)})
  ...
  return state.L, slope
end
lagrangian_lineslope!(αs, d, constraints, state, method) = lagrangian_lineslope(αs, d, constraints, state)

slopealpha(sx, gx, bstep, bgrad) =
  dot(sx, gx) +
  dot(bstep.slack_x, bgrad.slack_x) +
  dot(bstep.slack_c, bgrad.slack_c) +
  dot(bstep.λx, bgrad.λx) +
  dot(bstep.λc, bgrad.λc) +
...
function linesearch_anon(d, constraints, state, method::IPOptimizer{typeof(backtrack_constrained)})
  return αs -> lagrangian_linefunc!(αs, d, constraints, state, method)
end
...
barrier_value(constraints::AbstractConstraints, state) = barrier_value(constraints.bounds, state)
...
function equality_violation(f, bounds::ConstraintBounds, x, c, bstate::BarrierStateVars)
  return equality_violation(f, bounds, x, c, bstate.slack_x, bstate.slack_c, bstate.λx, bstate.λc, bstate.λxE, bstate.λcE)
end
...
equality_violation(bounds::ConstraintBounds, state::AbstractBarrierState) = equality_violation(identity, bounds, state)
...
function equality_grad!(gx, gsx, gsc, gλx, gλc, gλxE, gλcE, bounds::ConstraintBounds, x, c, J, sx, sc, λx, λc, λxE, λcE)
  ...
  return equality_grad_λ!(gλcE, c, bounds.valc, bounds.eqc)
end
equality_grad!(gx, gb::BarrierStateVars, bounds::ConstraintBounds, x, c, J, b::BarrierStateVars) = equality_grad!(
  gx,
  gb.slack_x,
  gb.slack_c,
  gb.λx,
...
function isfeasible(constraints, x)
  ...
  constraints.c!(c, x)
  return isfeasible(constraints, x, c)
end
...
function isinterior(constraints, x)
  c = Array{eltype(x)}(undef, constraints.bounds.nc)
  constraints.c!(c, x)
  return isinterior(constraints, x, c)
end
