function backtrack_constrained(
  ϕ,
  α::Real,
  αmax::Real,
  αImax::Real,
  Lcoefsα::Tuple{<:Real,<:Real,<:Real},
  c1::Real=0.5,
  ρ::Real=oftype(α, 0.5),
...
function backtrack_constrained_grad(
  ϕ,
  α::Real,
  αmax::Real,
  Lcoefsα::Tuple{<:Real,<:Real,<:Real},
  ...
  while α >= αmin
    ...
    if isfinite(val) &&
       val - (L0 + δval) <= c1 * abs(val - L0) &&
       (slopeα < c2 * abs(L1) || slopeα - (L1 + δslope) .<= c2 * abs.(slopeα - L1))
      return α
    end
    α *= ρ
  end
...
function evalgrad(slopeα, α, αI)
  return α * (slopeα[1] + slopeα[2] + slopeα[4]) + αI * slopeα[3]
end
