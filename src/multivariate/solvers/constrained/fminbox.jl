...
function BarrierWrapper(obj::NonDifferentiable, mu, lower, upper)
  ...
  return BarrierWrapper(obj, barrier_term, mu, copy(obj.F), copy(obj.F), nothing, nothing)
end
function BarrierWrapper(obj::OnceDifferentiable, mu, lower, upper)
  barrier_term = BoxBarrier(lower, upper)

  return BarrierWrapper(obj, barrier_term, mu, copy(obj.F), copy(obj.F), copy(obj.DF), copy(obj.DF))
end
...
function in_box(bb::BoxBarrier, x)
  return all(x -> x[1] >= x[2] && x[1] <= x[3], zip(x, bb.lower, bb.upper))
end
...
function gradient(bb::BoxBarrier, g, x)
  g = copy(g)
  return g .= _barrier_term_gradient.(x, bb.lower, bb.upper)
end
# Wrappers
...
function gradient!(obj::BarrierWrapper, x)
  ...
  return obj.DFtotal .= gradient(obj.obj) .+ obj.mu * obj.Fb
end
...
function initial_mu(obj::BarrierWrapper, F)
  ...
  gbarrier = map(
    x -> (isfinite.(x[2]) ? one(T) / (x[1] - x[2]) : zero(T)) + (isfinite(x[3]) ? one(T) / (x[3] - x[1]) : zero(T)),
  ...
  return obj.mu = initial_mu(gradient(obj.obj), gbarrier, T(F.mufactor), T(F.mu0))
end
...
function Fminbox(
  ...
  return Fminbox(method, promote(mu0, mufactor)..., precondprep) # default optimizer
end
...
barrier_method(m::ConjugateGradient, P, precondprep) =
  ConjugateGradient(eta=m.eta, alphaguess=m.alphaguess!, linesearch=m.linesearch!, P=P, precondprep=precondprep)

barrier_method(m::LBFGS, P, precondprep) =
  LBFGS(alphaguess=m.alphaguess!, linesearch=m.linesearch!, P=P, precondprep=precondprep)

barrier_method(m::GradientDescent, P, precondprep) =
  GradientDescent(alphaguess=m.alphaguess!, linesearch=m.linesearch!, P=P, precondprep=precondprep)

barrier_method(m::Union{NelderMead,SimulatedAnnealing,ParticleSwarm,BFGS,AbstractNGMRES}, P, precondprep) = m # use `m` as is
...
function optimize(
  ...
  return optimize(od, l, u, initial_x, F, options)
end

optimize(f, l::Number, u::Number, initial_x::AbstractArray{T}; kwargs...) where {T} =
  optimize(f, Fill(T(l), size(initial_x)...), Fill(T(u), size(initial_x)...), initial_x; kwargs...)
...
optimize(
  ...
  opt::Options=Options();
  kwargs...
) where {T} = optimize(f, Fill(T(l), size(initial_x)...), Fill(T(u), size(initial_x)...), initial_x, mo, opt; kwargs...)
optimize(
  f,
  l::AbstractArray,
  u::Number,
...
optimize(f, g, l::Number, u::Number, initial_x::AbstractArray{T}, opt::Options; kwargs...) where {T} =
  optimize(f, g, Fill(T(l), size(initial_x)...), Fill(T(u), size(initial_x)...), initial_x, opt; kwargs...)
optimize(f, g, l::AbstractArray, u::Number, initial_x::AbstractArray{T}, opt::Options; kwargs...) where {T} =
  optimize(f, g, l, Fill(T(u), size(initial_x)...), initial_x, opt; kwargs...)
optimize(f, g, l::Number, u::AbstractArray, initial_x::AbstractArray{T}, opt::Options; kwargs...) where {T} =
  optimize(f, g, Fill(T(l), size(initial_x)...), u, initial_x, opt; kwargs...)
...
function optimize(
  ...
  if F.method isa NelderMead
    for i in 1:length(state.f_simplex)
      x = state.simplex[i]
  ...
  while !converged && !stopped && iteration < outer_iterations
    ...
    results.x_converged, results.f_converged, results.g_converged, f_increased = assess_convergence(
      x,
      xold,
      minimum(results),
      fval0,
  ...
  return MultivariateOptimizationResults(
    ...
    results.f_abstol,
    results.f_reltol,
    f_abschange(minimum(results), value(dfbox)),
    f_relchange(minimum(results), value(dfbox)),
    results.g_converged,
    results.g_abstol,
    norm(g, Inf),
    results.f_increased,
