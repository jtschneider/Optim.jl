...
function Base.show(io::IO, r::MultivariateOptimizationResults)
  ...
  if isa(r.method, NelderMead)
    ...
    @printf io "    |x - x'|               = %.2e %s %.1e\n" x_abschange(r) x_abschange(r) <= x_abstol(r) ? "≤" : "≰" x_abstol(
      r
    )
    @printf io "    |x - x'|/|x'|          = %.2e %s %.1e\n" x_relchange(r) x_relchange(r) <= x_reltol(r) ? "≤" : "≰" x_reltol(
      r
    )
    @printf io "    |f(x) - f(x')|         = %.2e %s %.1e\n" f_abschange(r) f_abschange(r) <= f_abstol(r) ? "≤" : "≰" f_abstol(
      r
    )
    @printf io "    |f(x) - f(x')|/|f(x')| = %.2e %s %.1e\n" f_relchange(r) f_relchange(r) <= f_reltol(r) ? "≤" : "≰" f_reltol(
      r
    )
